...
#-------here is the change introduced:
        ...
        tkmessageBox(message="WARNING: singular gradient matrix at initial parameter estimates" , title = "FITTING STOPPED: exiting with non-optimized parameters!",  icon = "warning")
    }
    getPars.varying <- function() unlist(setNames(lapply(names(ind), get, envir = env), names(ind)))[useParams]
...
XPSnlsLM <- function (formula, data = parent.frame(), start, jac = NULL,
                  algorithm = "LM", control, lower = NULL, upper = NULL, 
  ...
  if (length(varNames)) {
     ...
     if (any(not.there <- n == -1)) {
        ...
        txt <- paste(" ***ERROR: Problems with Fit-Constraints on ", nnn,
                     "\n Cannot Make the Fit!",
                     "\n ==> DELETE and RE-SET Constraints and Try Again.\n\n", collapse="")
        tkmessageBox(message=txt, title="ERROR", icon="error")
        if (missing(start)) {
            txt <- paste("No starting values specified for some parameters.\n",
                         "Consider specifying the 'start' value", collapse="")
            tkmessageBox(message=txt, title="ERROR", icon="error")
    ...
    if (length(pnames) && any((np <- sapply(pnames, lenVar)) == -1)) {
       txt <- paste("fitting parameters ", paste(sQuote(pnames[np == -1]), collapse = ", "), 
                    " without any variables", collapse="")
       tkmessageBox(message=txt, title="ERROR", icon="error")
       n <- integer()
